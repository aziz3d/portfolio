...
# Copy the package.json and yarn.lock files to the working directory in the container.
...
# Run the project build command using yarn.
RUN yarn build
...
# Define the default command to be executed when the container is started with yarn start.
CMD ["yarn", "start"]